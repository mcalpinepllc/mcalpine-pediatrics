...
const plans: Plan[] = [
  ...
  { value: "amerigroup", label: "Amerigroup / Wellpoint Georgia", status: "confirm" },
  ...
  return (
    <div className="paper-ledger relative border-y border-live-oak/20 bg-white/72 px-5 py-7 shadow-[0_22px_60px_rgba(43,50,42,0.08)] sm:border sm:px-9 sm:py-9 sm:pl-12">
      ...
      <div className="mb-6 flex items-start gap-4">
        <img src="/manus-storage/mcalpine-sprouting-heart-a_851d8ff5.png" alt="" className="h-10 w-10 shrink-0 object-contain" />
        <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          ...
          <select
            ...
            required
            className="min-h-12 w-full rounded-none border-0 border-b border-live-oak/25 bg-transparent px-0 text-base text-foreground outline-none transition focus:border-coral-deep focus:ring-0"
          >
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="25.00"
              className="min-h-12 w-full rounded-none border-0 border-b border-live-oak/25 bg-transparent py-3 pl-7 pr-0 text-base text-foreground outline-none transition placeholder:text-foreground/35 focus:border-coral-deep focus:ring-0"
            />
          </div>
        </div>
        ...
        <button
          type="submit"
          className="button-press flex min-h-12 w-full items-center justify-center rounded-full bg-live-oak px-5 py-3 font-bold text-white shadow-[0_10px_24px_rgba(23,63,58,0.18)] transition hover:bg-coral-deep focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-coral/35"
        >
          Check plan & estimate
        </button>
      </form>
      ...
      <div className="mt-6 min-h-[92px]" aria-live="polite">
        {result ? (
          <div className={`border-y border-r border-l-4 p-4 ${result.plan.status === "listed" ? "border-live-oak/20 border-l-live-oak bg-sage/55" : "border-coral/30 border-l-coral bg-coral-soft"}`}>
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-bold text-live-oak">
                  {result.plan.status === "listed" ? "Directory listing found—not real-time eligibility" : "Please confirm with the office and insurer"}
                </p>
                <p className="mt-1 text-sm leading-6 text-foreground/75">
                  {result.plan.status === "listed"
                    ? `Dr. McAlpine appears in public listings for ${result.plan.label}. This is not a live eligibility or benefits check; network participation still varies by exact plan, employer, member, and date of service.`
                    : `We do not have a verified current directory listing for ${result.plan.label}. This does not mean the plan is declined. The office and insurer must confirm your exact product, network, eligibility, and benefits.`}
                </p>
                {result.copay !== null ? (
          ...
          <p className="flex items-start gap-2 text-sm leading-6 text-foreground/60">
            ...
            Insurance participation and patient responsibility can change. This tool reads no member data and performs no real-time eligibility check; it is a planning aid, not a coverage guarantee.
          </p>
        )}
      </div>
    </div>
